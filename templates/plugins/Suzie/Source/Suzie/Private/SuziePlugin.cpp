...
void FSuziePluginModule::InitializeDynamicClassForUE4(UDynamicClass* DynamicClass)
{
    // RoboQuest only needs the dynamic class object to exist and support default object creation.
    // The more elaborate Blueprint-generated-class setup that populates bindings/templates is not available
    // in the dump format, so keep this initializer intentionally minimal.
    if (DynamicClass)
    {
        DynamicClass->DynamicBindingObjects.Reset();
        DynamicClass->ComponentTemplates.Reset();
        DynamicClass->Timelines.Reset();
    }
}
#endif
...
void FSuziePluginModule::CreateDynamicClassesForJsonObject(const TSharedPtr<FJsonObject>& RootObject)
{
    ...
    for (auto It = (*Objects)->Values.CreateConstIterator(); It; ++It)
    {
        ...
        {
            UE_LOG(LogSuzie, Verbose, TEXT("Skipping problematic UE4 object: %s"), *ObjectPath);
            continue;
        }
#endif
        if (Type == TEXT("Class"))
    ...
    UE_LOG(LogSuzie, Display, TEXT("Finished processing %d reflected objects from dump"), (*Objects)->Values.Num());
}
...
UClass* FSuziePluginModule::FindOrCreateUnregisteredClass(FDynamicClassGenerationContext& Context, const FString& ClassPath)
{
    ...
    static const TPair<const TCHAR*, EClassFlags> ClassFlagNameLookup[] = {
        TPair<const TCHAR*, EClassFlags>(TEXT("CLASS_Abstract"), CLASS_Abstract),
        TPair<const TCHAR*, EClassFlags>(TEXT("CLASS_EditInlineNew"), CLASS_EditInlineNew),
        TPair<const TCHAR*, EClassFlags>(TEXT("CLASS_NotPlaceable"), CLASS_NotPlaceable),
        TPair<const TCHAR*, EClassFlags>(TEXT("CLASS_CollapseCategories"), CLASS_CollapseCategories),
        TPair<const TCHAR*, EClassFlags>(TEXT("CLASS_Const"), CLASS_Const),
        TPair<const TCHAR*, EClassFlags>(TEXT("CLASS_DefaultToInstanced"), CLASS_DefaultToInstanced),
        TPair<const TCHAR*, EClassFlags>(TEXT("CLASS_Interface"), CLASS_Interface),
    };
    ...
    for (const TPair<const TCHAR*, EClassFlags>& ClassFlagEntry : ClassFlagNameLookup)
    {
        if (ClassFlagNames.Contains(ClassFlagEntry.Key))
        {
            ClassFlags |= ClassFlagEntry.Value;
        }
    }

    // Allocate memory from ObjectAllocator for the class object and call the class constructor directly.
    // UE4 still expects runtime-generated classes to be UDynamicClass instances, while newer engine
    // versions carry additional static-function state directly on UClass.
#if ENGINE_MAJOR_VERSION < 5
    UDynamicClass* ConstructedClassObject = static_cast<UDynamicClass*>(GUObjectAllocator.AllocateUObject(sizeof(UDynamicClass), alignof(UDynamicClass), true));
    ::new (ConstructedClassObject)UDynamicClass(
        EC_StaticConstructor,
        *ClassName,
        ParentClass->GetStructureSize(),
        ParentClass->GetMinAlignment(),
        EClassFlags(ClassFlags | CLASS_CompiledFromBlueprint),
        CASTCLASS_None,
        UObject::StaticConfigName(),
        EObjectFlags(RF_Public | RF_Standalone | RF_Transient | RF_Dynamic | RF_MarkAsRootSet),
        ...
        &FSuziePluginModule::InitializeDynamicClassForUE4);
#else
    ...
    ConstructedClassObject->DeferredRegister(
#if ENGINE_MAJOR_VERSION < 5
...
UClass* FSuziePluginModule::FindOrCreateClass(FDynamicClassGenerationContext& Context, const FString& ClassPath)
{
    ...
    NewClass->Bind();
    NewClass->StaticLink();
    NewClass->ClassFlags |= CLASS_Constructed;
...
UScriptStruct* FSuziePluginModule::FindOrCreateScriptStruct(FDynamicClassGenerationContext& Context, const FString& StructPath)
{
    ...
    // Note that only flags that are set manually (e.g. non-computed flags) should be listed here
    static const TPair<const TCHAR*, EStructFlags> StructFlagNameLookup[] = {
        TPair<const TCHAR*, EStructFlags>(TEXT("STRUCT_Atomic"), STRUCT_Atomic),
        TPair<const TCHAR*, EStructFlags>(TEXT("STRUCT_Immutable"), STRUCT_Immutable),
    };
    ...
    for (const TPair<const TCHAR*, EStructFlags>& StructFlagEntry : StructFlagNameLookup)
    {
        if (StructFlagNames.Contains(StructFlagEntry.Key))
        {
            NewStruct->StructFlags = (EStructFlags)((int32)NewStruct->StructFlags | StructFlagEntry.Value);
        }
    }
...
UEnum* FSuziePluginModule::FindOrCreateEnum(FDynamicClassGenerationContext& Context, const FString& EnumPath)
{
    ...
    for (const TSharedPtr<FJsonValue>& EnumNameAndValueArrayValue : EnumNameJsonEntries)
    {
        ...
        if (EnumNameAndValueArray.Num() == 2)
        {
            ...
            EnumNames.Emplace(FName(*EnumConstantName), EnumConstantValue);
...
UFunction* FSuziePluginModule::FindOrCreateFunction(FDynamicClassGenerationContext& Context, const FString& FunctionPath)
{
    ...
    // Note that only flags that are set manually (e.g. non-computed flags) should be listed here
    static const TPair<const TCHAR*, EFunctionFlags> FunctionFlagNameLookup[] = {
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_Final"), FUNC_Final),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_BlueprintAuthorityOnly"), FUNC_BlueprintAuthorityOnly),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_BlueprintCosmetic"), FUNC_BlueprintCosmetic),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_Net"), FUNC_Net),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_NetReliable"), FUNC_NetReliable),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_NetRequest"), FUNC_NetRequest),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_Exec"), FUNC_Exec),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_Event"), FUNC_Event),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_NetResponse"), FUNC_NetResponse),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_Static"), FUNC_Static),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_NetMulticast"), FUNC_NetMulticast),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_UbergraphFunction"), FUNC_UbergraphFunction),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_MulticastDelegate"), FUNC_MulticastDelegate),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_Public"), FUNC_Public),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_Private"), FUNC_Private),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_Protected"), FUNC_Protected),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_Delegate"), FUNC_Delegate),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_NetServer"), FUNC_NetServer),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_NetClient"), FUNC_NetClient),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_BlueprintCallable"), FUNC_BlueprintCallable),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_BlueprintEvent"), FUNC_BlueprintEvent),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_BlueprintPure"), FUNC_BlueprintPure),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_EditorOnly"), FUNC_EditorOnly),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_Const"), FUNC_Const),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_NetValidate"), FUNC_NetValidate),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_HasOutParms"), FUNC_HasOutParms),
        TPair<const TCHAR*, EFunctionFlags>(TEXT("FUNC_HasDefaults"), FUNC_HasDefaults),
    };
    ...
    for (const TPair<const TCHAR*, EFunctionFlags>& FunctionFlagEntry : FunctionFlagNameLookup)
    {
        if (FunctionFlagNames.Contains(FunctionFlagEntry.Key))
        {
            FunctionFlags |= FunctionFlagEntry.Value;
        }
    }

    const FString FunctionClassPath = FunctionDefinition->GetStringField(TEXT("class"));

    // Delegate signatures are package-owned in the dump and must be created as delegate function types,
    // not as plain UFunction instances.
    UFunction* NewFunction = nullptr;
    if (FunctionClassPath == TEXT("/Script/CoreUObject.SparseDelegateFunction"))
    {
    ...
    else if (FunctionClassPath == TEXT("/Script/CoreUObject.DelegateFunction"))
    {
        NewFunction = NewObject<UDelegateFunction>(FunctionOuterObject, *ObjectName, RF_Public | RF_MarkAsRootSet | RF_ArchetypeObject);
    }
    else
    {
...
{
    // Note that only flags that are set manually (e.g. non-computed flags) should be listed here
    static const TPair<const TCHAR*, EPropertyFlags> PropertyFlagNameLookup[] = {
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_Edit"), CPF_Edit),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_ConstParm"), CPF_ConstParm),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_BlueprintVisible"), CPF_BlueprintVisible),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_ExportObject"), CPF_ExportObject),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_BlueprintReadOnly"), CPF_BlueprintReadOnly),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_Net"), CPF_Net),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_EditFixedSize"), CPF_EditFixedSize),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_Parm"), CPF_Parm),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_OutParm"), CPF_OutParm),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_ReturnParm"), CPF_ReturnParm),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_DisableEditOnTemplate"), CPF_DisableEditOnTemplate),
#if ENGINE_MAJOR_VERSION >= 5
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_NonNullable"), CPF_NonNullable),
#endif
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_Transient"), CPF_Transient),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_DisableEditOnInstance"), CPF_DisableEditOnInstance),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_EditConst"), CPF_EditConst),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_DisableEditOnInstance"), CPF_DisableEditOnInstance),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_InstancedReference"), CPF_InstancedReference),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_DuplicateTransient"), CPF_DuplicateTransient),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_SaveGame"), CPF_SaveGame),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_NoClear"), CPF_NoClear),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_SaveGame"), CPF_SaveGame),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_ReferenceParm"), CPF_ReferenceParm),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_BlueprintAssignable"), CPF_BlueprintAssignable),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_Deprecated"), CPF_Deprecated),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_RepSkip"), CPF_RepSkip),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_Deprecated"), CPF_Deprecated),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_RepNotify"), CPF_RepNotify),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_Interp"), CPF_Interp),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_NonTransactional"), CPF_NonTransactional),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_EditorOnly"), CPF_EditorOnly),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_AutoWeak"), CPF_AutoWeak),
        ...
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_ContainsInstancedReference"), CPF_ContainsInstancedReference),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_AssetRegistrySearchable"), CPF_AssetRegistrySearchable),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_SimpleDisplay"), CPF_SimpleDisplay),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_AdvancedDisplay"), CPF_AdvancedDisplay),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_Protected"), CPF_Protected),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_BlueprintCallable"), CPF_BlueprintCallable),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_BlueprintAuthorityOnly"), CPF_BlueprintAuthorityOnly),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_TextExportTransient"), CPF_TextExportTransient),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_NonPIEDuplicateTransient"), CPF_NonPIEDuplicateTransient),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_PersistentInstance"), CPF_PersistentInstance),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_UObjectWrapper"), CPF_UObjectWrapper),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_NativeAccessSpecifierPublic"), CPF_NativeAccessSpecifierPublic),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_NativeAccessSpecifierProtected"), CPF_NativeAccessSpecifierProtected),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_NativeAccessSpecifierPrivate"), CPF_NativeAccessSpecifierPrivate),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_SkipSerialization"), CPF_SkipSerialization),
#if (ENGINE_MAJOR_VERSION >= 5 && ENGINE_MINOR_VERSION >= 5)
        // Added in 5.5, allows references to the current object from within the property
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_AllowSelfReference"), CPF_AllowSelfReference),
#endif
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 6
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_RequiredParm"), CPF_RequiredParm),
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_TObjectPtr"), CPF_TObjectPtr),
#endif
        ...
        TPair<const TCHAR*, EPropertyFlags>(TEXT("CPF_HasGetValueTypeHash"), CPF_HasGetValueTypeHash),
    };
    ...
    for (const TPair<const TCHAR*, EPropertyFlags>& PropertyFlagEntry : PropertyFlagNameLookup)
    {
        if (PropertyFlagNames.Contains(PropertyFlagEntry.Key))
        {
            PropertyFlags |= PropertyFlagEntry.Value;
        }
    }
...
bool FSuziePluginModule::ParseObjectConstructionData(const FDynamicClassGenerationContext& Context, const FString& ObjectPath, FDynamicObjectConstructionData& ObjectConstructionData)
{
    ...
    static const TPair<const TCHAR*, EObjectFlags> ObjectFlagNameLookup[] = {
        TPair<const TCHAR*, EObjectFlags>(TEXT("RF_Public"), RF_Public),
        TPair<const TCHAR*, EObjectFlags>(TEXT("RF_Standalone"), RF_Standalone),
        TPair<const TCHAR*, EObjectFlags>(TEXT("RF_Transient"), RF_Transient),
        TPair<const TCHAR*, EObjectFlags>(TEXT("RF_Transactional"), RF_Transactional),
        TPair<const TCHAR*, EObjectFlags>(TEXT("RF_ArchetypeObject"), RF_ArchetypeObject),
        TPair<const TCHAR*, EObjectFlags>(TEXT("RF_ClassDefaultObject"), RF_ClassDefaultObject),
        TPair<const TCHAR*, EObjectFlags>(TEXT("RF_DefaultSubObject"), RF_DefaultSubObject),
    };
    ...
    for (const TPair<const TCHAR*, EObjectFlags>& ObjectFlagEntry : ObjectFlagNameLookup)
    {
        if (ObjectFlagNames.Contains(ObjectFlagEntry.Key))
        {
            ObjectConstructionData.ObjectFlags |= ObjectFlagEntry.Value;
        }
    }
    return true;
}
...
void FSuziePluginModule::PolymorphicClassConstructorInvocationHelper(const FObjectInitializer& ObjectInitializer)
{
    ...
    // Run logic necessary for the top level dynamic class object. That includes setting up default subobject overrides and the active archetype to use for property copying
    {
        ...
        for (const TPair<FName, UClass*>& SubobjectClassEntry : FinalSubobjectClasses)
        {
            // ReSharper disable once CppExpressionWithoutSideEffects
            ObjectInitializer.SetDefaultSubobjectClass(SubobjectClassEntry.Key, SubobjectClassEntry.Value);
        }
...
#else
            if (SubobjectOverrideData.SubobjectPath.Num() == 1)
            {
                ObjectInitializer.SetDefaultSubobjectClass(SubobjectOverrideData.SubobjectPath[0], SubobjectOverrideData.OverridenClass);
            }
            else
            {
                UE_LOG(LogSuzie, Verbose, TEXT("Skipping nested default subobject override on UE4 for class %s"), *DynamicClassHierarchyTree.Last()->GetPathName());
            }
#endif
        }
    }
